...
{
	public class SentryLevel2 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (targetNPC != null && ShootTimer <= 0)
			{
				...
				Vector2 bulletOrigin = new Vector2(Projectile.Center.X - 10, Projectile.Center.Y - 28);
				Vector2 shootVelocity = targetNPC.Center - bulletOrigin;

				shootVelocity.Normalize();
				shootVelocity *= 16;

				float spread = MathHelper.ToRadians(5.5f);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Vector2 gunOrigin = new Vector2(18, 14);

			Rectangle turretRectangle = new Rectangle(0, 42 * Projectile.frame, 66, 42); // turret rect in texture
			Rectangle standRectangle = new Rectangle(0, 84, 54, 34); // stand rect in texture
			...
			if (rotation < -MathHelper.PiOver2 || rotation > MathHelper.PiOver2)
			{
				...
				gunOrigin.X = turretRectangle.Width - 18;
			}
